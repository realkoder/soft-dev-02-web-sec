...
- Stored: Server stores → affects multiple users
- Reflected: Immediate reflection → one-time impact
- DOM-based: Client-side only → stealthier
...
server and included in _forms_ or _headers_**. When a state-changing request is made, the server verifies the token matches
what was issued to that session. _SameSite cookies_ provide another layer of defense by restricting when _cookies_ are sent
